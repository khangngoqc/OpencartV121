...
public class TC_FP_013_ResetPasswordHeadingTitleURLTest extends BaseClass {

	@Test(alwaysRun = false, dependsOnGroups = {"init_password"}, groups = { "master", "forgot password" })
	public void reset_password_heading_url_title_validate() throws IOException, MailosaurException {

		logger.info("******* Starting TC_FP_013_ResetPasswordHeadingTitleURLTest *******");
		...
		try {
			...
			WebElement pageHeading = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("heading")));
			
			if(pageHeading.isDisplayed()) {
				Assert.assertTrue(true, "Reset your Password heading is not displayed! " + lp.getHeadingReturningCustomer().getText());
			}
			
			
			logger.info("validate Page URL...");
			if(getDriver().getCurrentUrl().contains("password-reset")) {
				Assert.assertTrue(true, "Page URL is not displayed! " + getDriver().getCurrentUrl());
			}
			
			logger.info("validate Page Title...");
			if(getDriver().getTitle().equals("Reset your Password")) {
		...
		logger.info("******* Finished TC_FP_013_ResetPasswordHeadingTitleURLTest *******");
