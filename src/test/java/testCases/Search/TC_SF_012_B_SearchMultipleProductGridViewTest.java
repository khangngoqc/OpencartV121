...
public class TC_SF_012_B_SearchMultipleProductGridViewTest extends BaseClass {

	
	@Test(groups = {"master", "search"})
	void validate_multiple_search_results_Grid_view() {

		logger.info("******* Starting TC_SF_012_B_SearchMultipleProductGridViewTest *******");
		...
		try {
			...
			Assert.assertTrue(sp.getSearchResultCount()>1, "More than one products should be displayed in the search result page in Grid View");
			
			sp.clickAddToCart();
			Assert.assertTrue(sp.isAlertBannerDisplayed(), "Add to Cart button should work normally in Grid view.");
			Assert.assertTrue(sp.getAlertBannerText().contains("Success: You have added"), "Add to Cart button should work normally in Grid view.");
			
			sp.refreshPage();
			
			sp.clickAddToWishList();
			Assert.assertTrue(sp.isAlertBannerDisplayed(), "Add to Wish List button should work normally in Grid view.");
			Assert.assertTrue(sp.getAlertBannerText().contains("You must login"), "Add to Wish List button should work normally in Grid view.");
		...
		logger.info("******* Finished TC_SF_012_B_SearchMultipleProductGridViewTest *******");
