...
public class TC_FP_007_ResetPasswordFieldsTest extends BaseClass {

	@Test(alwaysRun = false, dependsOnGroups = {"init_password"}, groups = { "master", "forgot password" })
	public void reset_password_passwordfields_validation() throws IOException, MailosaurException {

		logger.info("******* Starting TC_FP_007_ResetPasswordFieldsTest *******");
		...
		try {
			...
			confirmPasswordInput.sendKeys("invalidPassword@123");

			WebElement saveButton = getDriver().findElement(By.id("save-btn"));
			saveButton.click();

			// Verify error Page/Banner
			WebElement errorMessage = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.className("alert-success")));
			Assert.assertTrue(errorMessage.getText().contains("Password and password confirmation do not match!"));
		...
		logger.info("******* Finished TC_FP_007_ResetPasswordFieldsTest *******");
